...
On the actual Playwright usage, it differs not much except that you have to run Chromium with the `--no-sandbox` argument because on Heroku there is the Chromium Sandbox not supported. For the full usage of Playwright, see on their official website [playwright.dev](https://playwright.dev).
