...
The results which Root Cause is recording gets saved automatically on your disk. You can view them by running `npx root-cause ls` to list them and `npx root-cause show <result-id>` to view a selected result. This will open the interface to see all the tests which were running, stack traces if there are failing tests, and additional metadata for network traffic, pages, or console as you can see here:
