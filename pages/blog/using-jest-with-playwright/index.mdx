import openGraphImage from './opengraph.png'
...
Also since this is a normal Jest configuration you can pass a `RegExp` which determines the files which Jest should run. It's useful to use e.g. a custom folder or file suffix as a naming convention to separate your unit tests from the end-to-end tests. See here as a reference for the [`testRegex`](https://jestjs.io/docs/en/configuration#testregex-string--arraystring) setting.
...
In the creation of the browser (`chromium.launch`) or the creation of the page (`browser.newPage`) you can passs options e.g. to start the browser as headful insetead of headless to manually click around or to ignore invalid SSL certificates which is useful for local testing.
...
Or you are also able to assert the value of input nodes on the page:
